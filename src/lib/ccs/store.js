...
export async function patchUserProfile(userId, body) {
  ...
  const nextProfile = { ...(typeof row.profile === "object" ? row.profile : {}) };

  for (const [k, v] of Object.entries(body || {})) {
    if (!PROFILE_KEYS.has(k)) continue;
    if (k === "handle") nextProfile.handle = String(v || "").replace(/[^\w.]/g, "_").slice(0, 32) || nextProfile.handle;
  ...
  await db.update(schema.ccsUsers).set({ profile: nextProfile }).where(eq(schema.ccsUsers.id, userId));

  return { profile: toPublicProfile(nextProfile) };
}
